...
app.get('/today', async (req, res) => {
    try {
      const todayReminders = await Reminder.find({ date: { $gte: new Date().setHours(0, 0, 0, 0) } });
...
app.patch('/update/:reminderId', async (req, res) => {
    try {
  ...
  app.delete('/complete/:reminderId', async (req, res) => {
    try {
      const reminderId = req.params.reminderId;
  
      const deletedReminder = await Reminder.findOneAndDelete({ _id: reminderId });
